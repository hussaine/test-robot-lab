...
def drive_forward():
    """Start moving forward only when not already doing so."""
    global drive_state

    forward_state = f"FORWARD {FOLLOW_SPEED}"
...
def send_steering(steering_angle, force=False):
    """Send meaningful steering changes, plus a forced straightening command."""
    global last_steering_angle

    if force or abs(steering_angle - last_steering_angle) >= SEND_THRESHOLD_DEG:
        robot.steer(steering_angle)
        last_steering_angle = steering_angle
        print(f"STEER {steering_angle:+.1f} deg")
    ...
    while True:
        ...
        if not faces:
            ...
            if not valid_distance:
                ...
                # Do not drive while turning toward an off-centre face.
                stop_drive()
                action = f"TURN {region}"
